...
const handleSearchListeners = ({ clearBtn, clearInput, focusOnHotkey, input, onValueChange }) => {
  if (!input || !clearBtn) {
    return () => null;
  }
...
const handleSearchExpansion = ({ collapse, collapsible, container, expand, input }) => {
  if (!input) {
    return () => null;
  }
...
const initSearchInput = (component, props = {}) => {
  ...
  const { onValueChange: externalOnValueChange } = props;
  if (!input || !clearBtn) {
    console.warn(
...
const handleListBoxListeners = ({
  ...
  const handleClick = (e) => {
    ...
    if (targetOption) {
      ...
      onOptionSelect({ e, value: targetValue });
    }
  };
...
const initListBox = (listbox, props = {}) => {
  var _a;
  const { container, input, onOptionSelect: externalOnOptionSelect, searchInput } = props;
  ...
  const onOptionSelect = ({ e, id, value }) => {
    var _a;
    ...
    if (externalOnOptionSelect) {
      externalOnOptionSelect({ id, value });
    }
  };
  const clearOptions = () => {
    options.forEach((option) => {
...
const initListBoxes = (props = {}) => {
  ...
  components.forEach((listbox) => {
    ...
    init$b(listbox, Object.assign(Object.assign({}, props), { searchInput }));
  });
};
  ...
  const handleKeyDown = (e) => {
    switch (e.key) {
      ...
      case 'Escape':
        ...
        break;
      case 'Tab':
        setExpanded(false, { autoFocus: false });
        break;
    }
    if (onKeyDown) {
      onKeyDown(e);
...
const renderFileErrors = (fileInput, error, iconRenderer) => {
  ...
  const statusIcon = iconRenderer('tds-icon-error-small-filled', { small: true });
...
const fileListItem = (fileInput, name, iconRenderer) => {
  ...
  if (fileInput.dataset.dtSupported) {
    const closeIcon = iconRenderer('tds-icon-close-small', { small: true });
    const removeFileButton = document.createElement('button');
    removeFileButton.appendChild(closeIcon);

    removeFileButton.addEventListener('click', () => {
      dispatchCustomEvent(fileInput, 'file-item-removed', { name });
...
const setFileList = (fileInput, files, fileNameToExclude, iconRenderer) => {
  ...
  const { dataTransfer, errors, fileNames } = setDataTransfer(
    files,
...
function updateRangeSliderHTMLState(input) {
  ...
  const { dec, max, min, value } = getRangeValues(input);
  ...
  if (caption) {
    const { tdsPercentage, tdsPrefix, tdsSuffix } = caption.dataset || {};
...
const positionIconBadge = (options) => {
  const { blockSize, dir, fontSize, iconWidth, label, positionOverride = { x: 0, y: 0 } } = options;
  ...
  const offset = { x: -1 - positionOverride.x, y: 0 - positionOverride.y };
...
const applyIconBadge = (icon, options) => {
  const { fontSize, iconHeight, iconWidth, id, label } = options;

  // get the positioning stuff we need for the badge shape and the text
  const { height, mode, textX, textY, width, x, y } = positionIconBadge(options);
...
function handleKeydown$1(e) {
  ...
  const volStep = 0.05;
  const seekStep = 5;
  const { altKey, ctrlKey, key, metaKey, shiftKey } = mediaPlayerEvent;
  ...
  if (e.currentTarget) {
    switch (key) {
      ...
      case 'c':
        toggleCaptions(
          !hasCaptions(mediaPlayerEvent.currentTarget),
      ...
      if (mediaPlayer instanceof MediaPlayer) {
        mediaPlayer._timeout = assignTimeout(
          ...
          () => {
            ...
            if (bottomHover) {
              const { bottom } = mediaPlayer.getBoundingClientRect();
  ...
  upgradeAttribute(attr) {
    ...
    Object.defineProperty(this, attr, {
      ...
      get: () => {
        ...
        switch (attr) {
          default:
            returnAttr = this.hasAttribute(attr);
            break;
        }
        return returnAttr;
      },
      set: (val = '') => {
        switch (attr) {
          default:
            return val ? this.setAttribute(attr, val) : this.removeAttribute(attr);
        }
      },
    });
  }
...
const { startTrappingFocus, stopTrappingFocus } = trapFocus$1;
...
const closeModal = (modal, props = {}) => {
  const { onCloseFinish, onCloseStart } = props;
...
const { getLangDir } = getDir;
const { moveBackdrop } = animateBackdrop$1;
/**
 * selectTab
 ...
 */
const selectTab = ({ enableTransitions, onChange, tabList, target }) => {
...
const initTabList = (tabList, props = {}) => {
  ...
  tabs.forEach((tab) => {
    ...
    const handleSelect = (e) => {
      ...
      selectPanel({ tabList, target: selectedTab });
    };
...
function getTooltipModifiers(tooltip) {
  ...
  const modifiers = { align: null, orientation: null, overlay };
...
const initTooltip = (tooltip, props) => {
  ...
  const { align, orientation, overlay } = getTooltipModifiers(tooltip);
