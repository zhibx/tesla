import { validateEmail } from '@tesla/validation';
...
import { getPatterns, getFormats } from '@tesla/intl-phone';
        ...
        if (field.name === 'phoneNumber' || field.name === 'phone') {
          if (
            (field.value.length >= 4 &&
              !this.validatePhone(field.value, countryCode, { format: 'national' })) ||
            field.value.length < 4
          ) {
            formValid = false;
          }
        ...
        if (!isChatLite) {
          ...
          for (const pair of formData.entries()) {
            if (chatLiteInterests.includes(pair[0])) {
              ...
              data.push({ name: pair[0], value: updatedValue });
            }
          }
        }
